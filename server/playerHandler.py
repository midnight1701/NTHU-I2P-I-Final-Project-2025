...
class Player:
    ...
    def update(self, x: float, y: float, map: str, direction) -> None:
...
class PlayerHandler:
    ...
    def register(self) -> int:
        with self._lock:
            ...
            # HINT: This part might be helpful for direction change
            # Maybe you can add other parameters?
            self.players[pid] = Player(pid, 0.0, 0.0, "", "down", time.monotonic())
            return pid
    ...
    def update(self, pid: int, x: float, y: float, map_name: str, direction) -> bool:
        with self._lock:
